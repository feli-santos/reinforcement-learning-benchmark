...
def evaluate_ppo(env_name, model_path, exp_id, num_episodes=3):
    logger.info(f"[{exp_id}] Starting PPO evaluation on {env_name}")
...
if __name__ == "__main__":
    ...
    for i, exp in enumerate(config["evaluate_experiments"]):
        if exp["algorithm"] == "ppo":
            model_path = os.path.join(exp["model_save_path"], "ppo_final")
            evaluate_ppo(exp["environment"], model_path, i)
