...
def train_a2c(env_name, total_timesteps, model_save_path, log_path, exp_id):
    logger.info(f"[{exp_id}] Starting A2C training on {env_name}")
    env = make_vec_env(env_name, n_envs=1, vec_env_cls=DummyVecEnv)
    checkpoint_callback = CheckpointCallback(
        save_freq=1000, save_path=model_save_path, name_prefix=f"a2c_model_{exp_id}"
    )
    model = A2C("MlpPolicy", env, verbose=1, tensorboard_log=log_path)
    model.learn(total_timesteps=total_timesteps, callback=checkpoint_callback)
    model.save(os.path.join(model_save_path, f"a2c_final_{exp_id}"))
